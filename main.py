...
parser.add_argument(
    ...
    type=str,
    default="relgraph",
    choices=["relgraph", "dual_stream"],
    help="relgraph: contrastive full stream + reconstruct-then-fuse missing stream over a relational graph",
)
parser.add_argument("--lambda_con", type=float, default=0.1, help="contrastive loss weight (full stream)")
parser.add_argument("--lambda_rec", type=float, default=0.5, help="reconstruction loss weight (missing stream)")
parser.add_argument("--lambda_kd", type=float, default=0.1, help="full->missing stream distillation weight")
parser.add_argument("--con_temperature", type=float, default=0.1, help="InfoNCE temperature")
parser.add_argument(
...
parser.add_argument(
    ...
    type=str,
    default="none",
    choices=["none", "wo_text_guided_fusion", "wo_lmmp", "wo_p_miss", "wo_p_mod"],
    help=(
        "model ablation used by the MSP-IMPROV study; training-stage ablations "
        "(without pre-training/fine-tuning) are controlled by the runner script"
    ),
)
