...
class MissingModalityPromptBank(nn.Module):
    ...
    def forward(
        ...
        missing_mask: torch.Tensor,
        use_missing_prompts: bool = True,
        use_modality_prompts: bool = True,
        ...
        if not use_modality_prompts:
            modality_prompt = torch.zeros_like(modality_prompt)
        if not use_missing_prompts:
            missing_prompt = torch.zeros_like(missing_prompt)
...
class DualStreamPromptLearningNetwork(nn.Module):
    ...
    def __init__(self, hyp_params):
        ...
        self.ablation = str(getattr(hyp_params, "ablation", "none")).lower()
        valid_ablations = {
            "none",
            "wo_text_guided_fusion",
            "wo_lmmp",
            "wo_p_miss",
            "wo_p_mod",
        }
        if self.ablation not in valid_ablations:
            raise ValueError(
                f"Invalid ablation={self.ablation}. Use one of {sorted(valid_ablations)}."
            )
    ...
    def forward(
        ...
        if self.ablation == "wo_lmmp":
            raw_modalities = torch.stack([text_feat, audio_feat, visual_feat], dim=1)
            modality_nodes = (
                raw_modalities * missing_mask.to(raw_modalities.dtype).unsqueeze(-1)
            )
            ...
            modality_nodes, processed_modalities = self.prompt_bank(
                ...
                visual_feat,
                missing_mask,
                use_missing_prompts=self.ablation != "wo_p_miss",
                use_modality_prompts=self.ablation != "wo_p_mod",
            )

        if self.ablation == "wo_text_guided_fusion":
            z_cross = modality_nodes.mean(dim=1)
