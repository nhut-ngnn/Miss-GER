...
def _build_model_from_state_dict(state_dict, hyp_params):
    ...
    has_relational_graph_weights = any(
        key.startswith(("full_graph.", "recon_graph."))
        for key in state_keys
    )
    ...
    if has_relational_graph_weights:
        model = mm.RelationalGraphPromptNetwork(hyp_params)
    elif has_prompt_weights and not has_dual_stream_weights:
