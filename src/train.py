...
def initiate(hyp_params, train_loader, valid_loader, test_loader):
    ...
    model_cls = (
        mm.RelationalGraphPromptNetwork
        if getattr(hyp_params, "model_type", "relgraph") == "relgraph"
        else mm.DualStreamPromptLearningNetwork
    )
...
def train_model(settings, hyp_params, train_loader, valid_loader, test_loader):
    ...
    def train_epoch(model, optimizer, criterion, epoch):
        ...
        use_relgraph_loss = getattr(model, "is_relational_graph", False)
        ...
        for i_batch, (batch_X, batch_Y, missing_mod) in enumerate(train_loader):
            ...
            if use_relgraph_loss:
                aux_losses = mm.relational_graph_auxiliary_loss(
                    aux_outputs,
                    lambda_con=float(getattr(hyp_params, "lambda_con", 0.1)),
                    lambda_rec=float(getattr(hyp_params, "lambda_rec", 0.5)),
                    lambda_kd=float(getattr(hyp_params, "lambda_kd", 0.1)),
                    temperature=float(getattr(hyp_params, "con_temperature", 0.1)),
                )
                total_loss = task_loss + aux_losses["loss"]
