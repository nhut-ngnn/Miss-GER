...
def _infer_msp_improv_video_from_audio(audio_path, video_base=None):
    """Resolve the corpus' asymmetric Audio/Video directory layout."""
    if not audio_path:
        return None
    ...
    audio_index = next(
        (index for index, part in enumerate(parts) if part.lower() == "audio"),
        None,
    )
    if audio_index is None:
        relative_parts = [os.path.basename(audio_path)]
        corpus_root = None
    else:
        relative_parts = parts[audio_index + 1 :]
        corpus_root = os.sep.join(parts[:audio_index]) or os.sep

    if (
        len(relative_parts) >= 2
        and relative_parts[0].lower() == relative_parts[1].lower()
    ):
        relative_parts.pop(1)
    relative_parts[-1] = os.path.splitext(relative_parts[-1])[0]

    roots = []
    if video_base:
        roots.append(str(video_base))
        if os.path.basename(os.path.normpath(str(video_base))).lower() != "video":
            roots.append(os.path.join(str(video_base), "Video"))
    if corpus_root:
        roots.append(os.path.join(corpus_root, "Video"))

    for root in roots:
        stem = os.path.join(root, *relative_parts)
        for candidate in [stem] + [f"{stem}{ext}" for ext in VIDEO_EXTENSIONS]:
            if os.path.exists(candidate):
                return candidate
    return None
...
def _load_frames_from_video_file(video_path, max_frames, start_time=None, end_time=None):
    ...
    # Some MSP-IMPROV AVI files have sparse or missing keyframe indexes. Random
    # seeking then returns no frames even though sequential decoding works.
    if not frames:
        cap = cv2.VideoCapture(video_path)
        decoded = []
        while cap.isOpened():
        ...
        sampled = _sample_indices(len(decoded), max_frames)
        for idx in sampled:
            rgb = cv2.cvtColor(decoded[idx], cv2.COLOR_BGR2RGB)
            frames.append(Image.fromarray(rgb))
...
def process_dataset(
    ...
    for item in tqdm(data, desc=f"Processing {os.path.basename(pkl_path)}"):
        ...
        if video_path is None and dataset_key == "msp-improv":
            video_path = _infer_msp_improv_video_from_audio(
                audio_path,
