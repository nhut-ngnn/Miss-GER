...
def _infer_msp_improv_video_path(data_root, wav_path, video_root=None):
    """Map an MSP-IMPROV utterance WAV to its matching utterance AVI.

    The distributed corpus uses paths such as
    ``Audio/session6/session6/S09H/R/<id>.wav`` for audio, but
    ``Video/session6/S09H/R/<id>.avi`` for video.
    """
    audio_root = os.path.join(data_root, "Audio")
    try:
        relative = os.path.relpath(wav_path, audio_root)
    except (TypeError, ValueError):
        relative = os.path.basename(wav_path)

    parts = list(os.path.normpath(relative).split(os.sep))
    ...
    if video_root:
        ...
        if os.path.basename(os.path.normpath(video_root)).lower() != "video":
            roots.append(os.path.join(video_root, "Video"))
    roots.append(os.path.join(data_root, "Video"))

    candidates = []
    for root in roots:
        candidates.extend(_build_video_candidates(root, [os.path.join(*parts)]))
    return _first_existing_path(candidates)
...
def preprocess_msp_improv(args):
    ...
    for wav_path in tqdm.tqdm(wav_files, desc="Processing MSP-IMPROV"):
        ...
        video_path = _infer_msp_improv_video_path(
            args.data_root,
            wav_path,
            video_root=args.video_root,
        )
        if video_path is None:
            logging.warning("Cannot find MSP-IMPROV video for %s", wav_path)
        samples.append(
            _build_sample(
                sample_id=sample_id,
                audio_path=wav_path,
