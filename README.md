...
- **Relational graph dual stream** (`--model_type relgraph`, default): modalities are graph nodes with typed directed relations. A *full-modality stream* is trained with cross-modal contrastive learning; a *missing-modality stream* does reconstruct-then-fuse (missing nodes are reconstructed from available ones by relational message passing, then fused). Each sample is routed by its availability mask. Loss weights: `--lambda_con`, `--lambda_rec`, `--lambda_kd`, `--con_temperature`.
...
### MSP-IMPROV ablation study

Run the complete Prompt4MER setting and the six paper ablations with:

```bash
bash scripts/ablate_msp_improv.sh
```

The runner covers `w/o Text-Guided Fusion`, `w/o LMMP`, `w/o P_miss`,
`w/o P_mod`, `w/o pre-training`, and `w/o fine-tuning`. Checkpoints, evaluation
